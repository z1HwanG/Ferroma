...
use ferroma_core::{AttachmentId, MailboxId, MessageId};
...
impl MessagesRepository {
    ...
    pub async fn move_to_folder(
        ...
        let moved = sqlx::query_as::<_, Message>(
            "WITH next_uid AS (
             ...
             UPDATE messages m
                SET folder_id = $2, mailbox_id = $3, uid = next_uid.uid, updated_at = NOW()
               FROM next_uid
              WHERE m.id = $1
             RETURNING m.*",
        )
        ...
        match moved {
            ...
            None => Err(self.missing_message_or_folder(id, to_folder_id).await?),
        }
    }
        ...
        let inserted: Option<(i64,)> = sqlx::query_as(
            "WITH next_uid AS (
             ...
             SELECT $2, $3, next_uid.uid, m.rfc_message_id, m.thread_id, m.subject, m.sender,
                    m.sender_name, m.snippet, m.size_bytes, m.storage_path, m.checksum_sha256,
    ...
    pub async fn set_snippet(&self, id: MessageId, snippet: Option<&str>) -> Result<()> {
        let done = sqlx::query("UPDATE messages SET snippet = $2, updated_at = NOW() WHERE id = $1")
            .bind(id.get())
            .bind(snippet)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn search(&self, query: MessageSearch) -> Result<Vec<Message>> {
        let mut builder = QueryBuilder::<Postgres>::new("SELECT * FROM messages WHERE expunged_at IS NULL");

        if let Some(folder_id) = query.folder_id {
            builder.push(" AND folder_id = ").push_bind(folder_id.get());
        }
        if let Some(mailbox_id) = query.mailbox_id {
            builder.push(" AND mailbox_id = ").push_bind(mailbox_id.get());
        }
        ...
        Ok(builder.build_query_as::<Message>().fetch_all(&self.pool).await?)
    }
