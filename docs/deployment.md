...
| 5. Build the image | a local `docker build` (10–30 minutes the first time). Pass `--image wesukilaye/ferroma:0.1.17` to pull the release instead — the same command skips the build entirely |
...
| `FERROMA_VERSION` | `0.1.17` | prod (`:?`) | a released image tag; prod never builds |
...
a client older than the server aborts before it writes anything. An import refuses
an archive whose `pg_dump` major version is not the server's: a dump written by a
newer client cannot be loaded by an older server.
