...
| 5. Build the image | a local `docker build` (10–30 minutes the first time). Pass `--image wesukilaye/ferroma:0.1.13` to pull the release instead — the same command skips the build entirely |
...
| `FERROMA_VERSION` | `0.1.13` | prod (`:?`) | a released image tag; prod never builds |
