...
| 5. Build the image | a local `docker build` (10–30 minutes the first time). Pass `--image wesukilaye/ferroma:0.1.7` to pull the release instead — the same command skips the build entirely |
...
| `FERROMA_VERSION` | `0.1.7` | prod (`:?`) | a released image tag; prod never builds |
...
```bash
# Roll the image back.
sed -i 's/^FERROMA_VERSION=.*/FERROMA_VERSION=0.1.7/' .env
docker compose -f docker-compose.prod.yml pull ferroma
docker compose -f docker-compose.prod.yml up -d ferroma
```
...
agree — a `v0.2.0` tag on a tree that says `0.1.7` fails before anything is built —
