...
| 5. 构建镜像 | 本机 `docker build`（首次 10–30 分钟）。加上 `--image wesukilaye/ferroma:0.1.14` 改为拉取已发布版本——同一条命令会完全跳过构建 |
...
| `FERROMA_VERSION` | `0.1.14` | prod（`:?`） | 已发布的镜像标签；prod 从不构建 |
...
```bash
# 把镜像回滚。
sed -i 's/^FERROMA_VERSION=.*/FERROMA_VERSION=0.1.14/' .env
docker compose -f docker-compose.yml pull ferroma
docker compose -f docker-compose.yml up -d ferroma
```
