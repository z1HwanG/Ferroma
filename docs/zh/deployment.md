...
| 5. 构建镜像 | 本机 `docker build`（首次 10–30 分钟）。加上 `--image wesukilaye/ferroma:0.1.7` 改为拉取已发布版本——同一条命令会完全跳过构建 |
...
| `FERROMA_VERSION` | `0.1.7` | prod（`:?`） | 已发布的镜像标签；prod 从不构建 |
...
`0.1.7` 的树上打 `v0.2.0` 标签会在构建任何东西之前失败——再跑
