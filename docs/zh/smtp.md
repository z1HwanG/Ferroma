...
`[policy.greylist]` **默认关闭**。开启后，未经认证的对端若其
`(地址, 发件人, 收件人)` 三元组从未出现过，会被以 `451 4.7.1` 延迟一次。真正的
MTA 会把邮件排队并重试；大多数群发者不会，价值就在于此。延迟从**首次**出现
起算，且该时间戳永不被改写，因此过早重试的对端无法把自己等待的截止时间往后推。
...
|---|---|
| 已认证会话 | 它是已知用户，不是未知对端 |
| 空反向路径（`<>`） | 退信没有可用于重试的地址，延迟它等于丢弃报告 |
| `whitelist` 中的地址或网段 | 不排队的中继或伙伴，否则每出现一个新三元组就要吃一次延迟 |

检查发生在 `RCPT TO`，且仅在地址解析到真实邮箱之后：对不存在的地址延迟，是没有
任何东西支撑的等待。它**失败即放行**——数据库出错时接受邮件并记警告。灰名单用于
过滤群发者，绝不应成为对端无法投递邮件的原因。

三元组由 `ferroma storage gc` 在 `retention_days`（默认 30 天）后清理。忘掉一个
三元组的代价是多一次延迟，绝不会丢信。
