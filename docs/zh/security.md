...
每次刷新都会轮换：出示的那个会话被吊销，并开一个同类型的新会话。交接是原子
的——`SessionsRepository::rotate`在同一个事务里吊销旧行并插入新行，因此携带同一
枚令牌的两个并发请求不可能都拿到有效的新令牌。输掉条件更新的那个请求会被当作
复用拒绝，但不会烧掉整个家族——赢家可能是同一客户端自己的重试，良性的竞态不
该杀死合法的新令牌。烧家族只留给读回时已经是吊销状态的重放，那才是真正的盗用
信号。
...
代码使用固定的虚拟 PHC 哈希来实现这一点：未知账号先对 `DUMMY_PASSWORD_HASH`
做一次 Argon2 校验，再返回`invalid_credentials()`。信息与 Argon2 开销都与密码
错误一致，因此登录端点既不通过响应、也不通过耗时枚举账号；两种情况下每次
尝试都恰好花掉一次校验，这也正是上面的限流必须存在的原因。
...
* **注册不等于强制执行。** 没有被验证码证明过的密钥不是第二因子，API 与 Webmail 都会
  如实这样说，而不是报告账号并不具备的保护。
* **关闭它需要账号密码。** 仅有一个有效会话不够，两个界面都如此——被盗的会话正是该因子
  存在的理由。
* **任何管理员都无法清除第二因子。** Admin API 与控制台都不提供。否则一个被盗的管理员
  会话就能绕过服务器上每一个账号的第二因子。对于同时丢失验证器与恢复码的用户，恢复路径
  是主机上的 `ferroma user totp-disable`（见 [deployment.md](deployment.md) §6.6），
  它要求 shell 访问权限。
* **管理员可以吊销应用专用密码**，因为丢失的设备必须能够停止工作。这不会削弱任何东西：
  第二因子保持不变。

剩下的风险是工程上无法消除的那一个：同时丢失两条恢复路径的用户需要一位能访问主机的
运维者，而一位不经验证就按要求执行 `totp-disable` 的运维者，本身就成了那个绕过口。这两
半都写在部署指南里，而不是留作隐含前提。
